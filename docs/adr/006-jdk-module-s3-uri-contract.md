...
credentials) is read from the `env` map keys `region` / `endpoint` /
`credentialsProvider`. No other URI component is interpreted.
...
```
s3://<bucket>[/<key>][?region=<region>&endpoint=<uri>]
```
...
- `region` — a region id, e.g. `eu-central-1`
- `endpoint` — an endpoint override URI, e.g. `http://localhost:9000`
...
userinfo. They are supplied only through the `env`-map key `credentialsProvider`, as an
`AwsCredentialsProvider` object.

### Resolution precedence

For `region` and `endpoint`, in order:
...
Path p = Path.of(URI.create("s3://my-bucket/reports/q3.csv?region=eu-central-1"));

// Local MinIO — endpoint in the URI, credentials (only) in the env map:
FileSystems.newFileSystem(
        URI.create("s3://my-bucket?endpoint=http://localhost:9000&region=us-east-1"),
        Map.of("credentialsProvider", staticProvider));

// env map wins over the URI when both set the same key:
FileSystems.newFileSystem(
        URI.create("s3://my-bucket?region=eu-central-1"),
        Map.of("region", "eu-west-1"));   // → eu-west-1
```
...
2. **Full commons-vfs parity** — credentials in userinfo (`accessKey:secretKey[:region]@host`),
   ...
   keeps clean.
3. **Different names for the query vs. the env map** (e.g. `?region=` in the URI but `regionId`
